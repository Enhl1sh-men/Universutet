...
int main() {
    // Тест класу Room та операторів + і -
    Room room1("Deluxe", 2, 150.0);
    Room room2("Standard", 1, 100.0);

    Room room3 = room1 + room2;
    room3.displayRoomInfo();

    Room room4 = room1 - room2;
    room4.displayRoomInfo();

    // Тест класу AdditionalServices та операторів +=, -=, *, []
    ...
    AdditionalServices multipliedServices = services * 2;
    multipliedServices.displayServices();

    std::cout << "Service at index 1: " << services[1] << std::endl;

    // Тест шаблонного класу HotelBilling
    std::vector<std::string> clients = { "Client 1", "Client 2", "Client 3" };
    HotelBilling<std::string, double, double, double, double> billing(clients, 500.0, 50.0, 0.1, 20.0);
    billing.displayTotalAmount();

    int choice;
    bool exit = false;

    Room roomStatic("Suite", 2, 150.0); // статичні
...
// Функція для роботи з файлами 8
void Service(const HotelReservation& reservation) {
    try {
